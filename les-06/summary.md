# Les 6 - Summary

## Source

- Based on: Miro board screenshot with sections for fonetiek, grammatica, woordenschat, getallen, luisteren, spreken, schrijven, and huiswerk.
- Date added: 2026-07-05

## Main Topics

- Demonstrative pronouns: `dit`, `dat`, `deze`, `die`.
- Present tense verb forms.
- Classroom vocabulary and lesson activities.
- Numbers, including tens and larger numbers.
- Pronunciation contrasts: `ui`, `ij/ei`, and `eu`.
- Short classroom story: going to school, greeting the teacher, break, going outside.

## Grammar

### Demonstrative Pronouns

Dutch demonstratives depend on article type and distance.

| Article type | Nearby | Far away | Example |
| --- | --- | --- | --- |
| het-word | dit | dat | Dit boek is hier. Dat boek is daar. |
| de-word | deze | die | Deze tas is hier. Die tas is daar. |
| plural | deze | die | Deze boeken zijn hier. Die boeken zijn daar. |

Useful distance words:

- `dichtbij` = nearby
- `ver weg` = far away
- `hier` = here
- `daar` = there

### Present Tense

The lesson practices simple present tense forms.

| Subject | Example |
| --- | --- |
| ik | Ik woon in Nederland. |
| jij/u/hij/zij | Jij spreekt Nederlands. Hij gaat naar school. |
| wij/jullie/zij | Wij luisteren naar de docent. De cursisten schrijven woorden. |

Common lesson verbs:

- `wonen`
- `spreken`
- `gaan`
- `luisteren`
- `schrijven`
- `lezen`
- `maken`
- `beantwoorden`

## Vocabulary Themes

- Classroom: `de school`, `de docent`, `de cursist`, `de tekst`, `het woord`, `het boek`, `de pen`, `de computer`, `het bord`.
- Objects for demonstratives: `de tas`, `de tafel`, `de stoel`, `de fiets`, `het huis`, `de appel`.
- Lesson actions: `lezen`, `luisteren`, `schrijven`, `spreken`, `maken`, `beantwoorden`.
- Places and movement: `binnen`, `buiten`, `naar school`, `naar binnen`, `naar buiten`.
- Descriptions: `groot`, `klein`, `mooi`, `nieuw`, `oud`, `schoon`, `interessant`.

## Numbers

The lesson includes numbers from 0 to 100 and examples of larger numbers.

Examples:

- `nul`, `een`, `twee`, `drie`
- `elf`, `twaalf`, `dertien`, `veertien`, `vijftien`
- `twintig`, `dertig`, `veertig`, `vijftig`
- `eenentwintig`, `tweeëntwintig`, `drieënzeventig`
- `honderd`, `tweehonderd`, `duizend`

## Pronunciation

Practice visible sound groups:

- `ui`: `huis`, `tuin`, `Duits`, `buiten`, `luisteren`
- `ij/ei`: `mijn`, `zijn`, `klein`, `trein`, `schrijft`
- `eu`: `deur`, `neus`, `leuk`, `keuken`, `beurt`

Useful contrast pairs:

- `bui` - `bij`
- `huis` - `hijs`
- `tuin` - `trein`
- `leuk` - `luik`
- `keus` - `kuis`

## Listening And Speaking

The lesson includes short listening tasks about people in class and a classroom situation.

Useful speaking questions:

1. Waar woon jij?
2. Spreek jij Nederlands?
3. Wat doe jij in de les?
4. Schrijf jij met een pen of op de computer?
5. Wat doe jij in de pauze?

## Writing Practice

The screenshot includes translation-style writing practice with sentences about school and class.

Useful examples:

1. Amir gaat naar school.
2. De school is groot.
3. Amir gaat naar binnen.
4. De docent zegt: "Goedemorgen!"
5. Het is nu pauze.
6. De studenten gaan samen naar buiten.
7. De studenten leren Nederlands.
8. Zij luisteren naar de docent.
9. De docent schrijft woorden.
10. Zij maken opdrachten op de computer.

## Homework Ideas

- Choose `dit`, `dat`, `deze`, or `die`.
- Mark sentences as `dichtbij` or `ver weg`.
- Fill in present tense verb forms.
- Correct wrong present tense sentences.
- Translate short classroom sentences into Dutch.
- Write numbers as words.
- Read pronunciation rows aloud and record yourself.
- Answer simple speaking questions in full sentences.

## Unclear From Source

- Some smaller listening exercise text is not fully readable from the screenshot.
- Some pronoun drill cards are too small to transcribe exactly.
