# Les 18 - Huiswerk van het bord — **По-русски:** Урок 18 - Домашнее задание с доски

> **Русская версия:** название каждого задания продублировано по-русски. Основные отдельные инструкции также продублированы строкой `По-русски`. Учебные предложения сохранены на исходном языке, чтобы не раскрывать ответы в заданиях на перевод.



> **Пояснение по-русски:** Работа закрепляет форму прилагательного, одежду, аксессуары и внешность.

De screenshots tonen geen apart huiswerkvak. Hieronder staan de zichtbare grammatica- en begripsopdrachten uit het lesmateriaal.

## 1. Vul de goede vorm van het adjectief in — **По-русски:** 1. Вставьте правильную форму прилагательного

Voorbeeld / Пример: `(leuk)` Het Museum van Speelklok tot Pierement is een **leuk** museum.

1. `(oud)` Ik eet dit brood niet. Het is __________.
2. `(wit)` Zij koopt een __________ jurk.
3. `(groot)` Ik kom uit een __________ gezin.
4. `(groot)` Ik heb een __________ familie.
5. `(interessant)` Hij leest een __________ boek.
6. `(oud)` Mijn buurman is een __________ man van 83 jaar.
7. `(rood)` De leraar corrigeert de test met een __________ pen.
8. `(groen)` De bananen zijn niet rijp, ze zijn __________.
9. `(vers)` Ik wil graag een __________ appeltaart.
10. `(goedkoop)` Verkoopt u __________ tweedehands fietsen?
11. `(bruin)` Ze houdt niet van __________ brood.
12. `(zoet)` Eén kilo __________ mandarijnen, alstublieft.

## 2. Beantwoord de vragen over kleding — **По-русски:** 2. Ответьте на вопросы об одежде

1. Wie heeft een jurk aan?
2. Welke kleur heeft de jurk?
3. Wie heeft bruine schoenen aan?
4. Wie zijn vandaag vrij?
5. Wat gaat Ellis doen?
6. Wie heeft een pak aan?
7. Waarom heeft hij een pak aan?
8. Welke kleur heeft het pak?
9. Wie hebben een trui aan?
10. Wie draagt nette kleren?

## 3. Kies het goede antwoord over accessoires — **По-русски:** 3. Выберите правильный ответ об аксессуарах

1. Waar draag je een hoed?  
   a. op je hoofd  
   b. op je neus  
   c. op je arm
2. Waar draag je een bril?  
   a. op je hoofd  
   b. op je neus  
   c. in je oor
3. Wat draag je aan je handen?  
   a. alleen een muts  
   b. alleen handschoenen  
   c. een muts en handschoenen
4. Welke kleur hebben de handschoenen?  
   a. wit  
   b. blauw  
   c. zwart
5. Welke kleur heeft de hoofddoek?  
   a. wit  
   b. blauw  
   c. zwart
6. Wat draag je om je arm?  
   a. een ring en een horloge  
   b. een ketting en een ring  
   c. een horloge en een armband
7. Waar draag je een ring?  
   a. om je nek  
   b. om je vinger  
   c. om je arm
8. Wat draag je om je nek?  
   a. alleen een sjaal  
   b. alleen een ketting  
   c. een sjaal en een ketting

## Antwoorden — **По-русски:** Ответы

### 1

1. oud
2. witte
3. groot
4. grote
5. interessant
6. oude
7. rode
8. groen
9. verse
10. goedkope
11. bruin
12. zoete

### 2

1. Ellis heeft een jurk aan.
2. De jurk is rood / donkerrood.
3. Erik heeft bruine schoenen aan.
4. Jens en Jacob zijn vandaag vrij.
5. Ellis gaat naar een feest.
6. Erik heeft een pak aan.
7. Hij heeft een pak aan omdat hij naar zijn werk gaat.
8. Het pak is donkerblauw / blauw.
9. Jens en Jacob hebben allebei een trui aan.
10. Erik draagt nette kleren.

### 3

1. a - op je hoofd
2. b - op je neus
3. b - alleen handschoenen
4. c - zwart
5. c - zwart
6. c - een horloge en een armband
7. b - om je vinger
8. c - een sjaal en een ketting
